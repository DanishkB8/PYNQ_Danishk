...
typedef struct {
    ...
    XAm_accel_uio_map maps[ MAX_UIO_MAPS ];
} XAm_accel_uio_info;

/***************** Variable Definitions **************************************/
static XAm_accel_uio_info uio_info;
...
static int uio_info_read_name(XAm_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAm_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAm_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAm_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAm_accel_Initialize(XAm_accel *InstancePtr, const char* InstanceName) {
	XAm_accel_uio_info *InfoPtr = &uio_info;
